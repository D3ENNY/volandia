...
$(()=>{
    ...
    $.getJSON(currentFilePath+"validatorJSON/editFly.json", (data) =>{
        localStorage.setItem("addFly", cryptManager.encrypt(JSON.stringify(data)))
    })
...
let checkValidate = () =>{
    let json = JSON.parse(cryptManager.decrypt(localStorage.getItem("addFly")))
    let flag = true
    console.log(json);
    $.each(json, (i, data) =>{
        if(data != true) flag=false
    })
    if(flag) $("#addFly").setEnabled()
    else $("#addFly").setDisabled()
}
...
$.fn.setError = function(){
    ...
    localStorage.setItem("addFly", cryptManager.encrypt(JSON.stringify(json)))
}
